...
const words = await fetch("/assets/pgyft/words.json")
  .then((response) => response.json())
  .then((words) => {
    return Object.fromEntries(
      Object.entries(words).map(([key, val]) => [key, new Set(val)]),
    );
  });

/** @type {{[k: string]: Entry}} */
const raw_posts = await fetch("/assets/pgyft/posts.json").then((response) =>
  response.json(),
);
const posts = Array.from(
  Object.entries(raw_posts).map(([key, entry]) => ({
    title: key,
...
function getFilteredPost() {
  ...
  if (search.length == 0) {
    ...
    const containingPostKeys = Object.entries(words)
      .filter(([_, val]) => {
...
function rerender() {
  ...
  const sortedPosts = posts.toSorted((a, b) =>
    sortByDate(a, b, getSortDirection()),
  );
